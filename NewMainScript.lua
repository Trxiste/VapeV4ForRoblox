...
local function removetag(char)
	...
	for _, obj in char:GetDescendants() do
		if obj:IsA('BillboardGui') and (obj.Name == 'VapeOwnerTag' or obj.Name == 'VapeTag') then
			obj.Enabled = false
			obj:Destroy()
		elseif obj:IsA('TextLabel') and obj.Text == 'VAPE OWNER' then
			local billboard = obj:FindFirstAncestorWhichIsA('BillboardGui')
			if billboard then
