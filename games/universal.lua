...
run(function()
	...
	local function applyFlags(flags)
		...
		for k, v in pairs(flags) do
			local stripped = stripFlagPrefix(k)
	...
	local function resetFlags()
		...
		local resetValues = {
			...
			["DFIntInterpolationNumMechanismsPerTask"] = "10",
			["DFIntInterpolationNumParallelTasks"] = "10",
			["DFIntMaxInterpolationRecursionsBeforeCheck"] = "5",
			...
			["DFIntInterpolationMinAssemblyCount"] = "5",
			["DFIntNumFramesToKeepAfterInterpolation"] = "3",
			["DFIntInterpolationNumMechanismsBatchSize"] = "5",
		}
	...
	NoDelay = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				if not setfflag then
					vape:CreateNotification('NoDelay', 'Your executor does not support setfflag', 3)
					NoDelay:Toggle()
					return
				end
		...
		Tooltip = 'Reduces interpolation delay for smoother gameplay'
	})
...
run(function()
	...
	local LocalPlayer = Players.LocalPlayer
	local RemoveAllRestrictions
	local Connection = nil
	local Enabled = false
	
	                                                
	local fatigueFolders = {
		"KickCD",
		"NoKickFrame",
		"ShotLowered",
		"JustUsedSkill",
	...
	local function deleteFatigueFolders()
		...
		for _, name in ipairs(fatigueFolders) do
			local folder = status:FindFirstChild(name)
			if folder then
				folder:Destroy()
			end
		end
		
		for _, name in ipairs(gameStateFolders) do
	...
	local function hookStatus()
		...
		status.ChildAdded:Connect(function(folder)
			for _, name in ipairs(fatigueFolders) do
	...
	local function hookBallStatus()
		local ballStatus = Workspace:FindFirstChild("ballStatus")
		if not ballStatus then return end
		
		ballStatus.ChildAdded:Connect(function(folder)
			if folder.Name == "NoTapInFrames" or 
			   folder.Name == "IFrame" or 
			   folder.Name == "JustSlideTackled" or
			   folder.Name == "JustChipped" or
			   folder.Name == "TapFrames" then
				task.wait(0.01)
				if folder.Parent then
					folder:Destroy()
				end
	...
	local function removeStaminaDrain()
		local character = LocalPlayer.Character
		if not character then return end
		...
		local stamina = stats:FindFirstChild("Stamina")
		if stamina then
			stamina.Value = math.max(stamina.Value, 80)
		end
	...
	local function runLoop()
		while Enabled do
			deleteFatigueFolders()
			removeStaminaDrain()
			task.wait(0.05)
		end
	end
	
	RemoveAllRestrictions = vape.Categories.Blatant:CreateModule({
		Name = 'RemoveAllRestrictions',
		Function = function(callback)
			Enabled = callback
			...
			if callback then
				...
				vape:CreateNotification('No Fatigue', 'All fatigue removed!', 2)
			else
				if Connection then
					coroutine.close(Connection)
					Connection = nil
		...
		Tooltip = 'Remove 99% of the game restrictions PS: THIS DOES NOT TURN OFF'
	})
	
	RemoveAllRestrictions:Clean(function()
		Enabled = false
		if Connection then
			coroutine.close(Connection)
			Connection = nil
...
run(function()
	local M2Tap = vape.Categories.Utility:CreateModule({
		Name = 'M2Tap',
		Function = function(callback)

		end,
		Tooltip = 'When the Ball is stolem, quickly press M2 to try and take it back'
	})
	
	M2Tap:Clean(function()
...
run(function()
    ...
    local CurrentIntensity = "Medium"
    local BypassesEnabled = true
    ...
    local intensitySettings = {
        Low = { cacheSize = 3, spoofRate = 0.3 },
        Medium = { cacheSize = 8, spoofRate = 0.6 },
        High = { cacheSize = 15, spoofRate = 0.9 }
    }
    ...
    local function setupBypasses()
        if not BypassesEnabled then return end

                                       
        if checkcaller then
            local orig = checkcaller
            hookfunction(checkcaller, function() return true end)
            originalFunctions.checkcaller = orig
        end

                                
        if getscriptbytecode then
            local orig = getscriptbytecode
            hookfunction(getscriptbytecode, function(script)
                if script and script.Name and (script.Name:lower():find("anticheat") or script.Name:lower():find("detect")) then return "" end
                return orig(script)
            end)
            originalFunctions.getscriptbytecode = orig
        end

                                             
        if getconnections then
            local orig = getconnections
            hookfunction(getconnections, function(signal)
                local conns = orig(signal)
                local filtered = {}
                for _, c in ipairs(conns) do
                    if not checkcaller or not checkcaller(c.Function) then table.insert(filtered, c) end
                end
                return filtered
            end)
            originalFunctions.getconnections = orig
        end
    end

    local function startDesync(params)
        ...
        desyncConnection = RunService.Heartbeat:Connect(function()
            ...
            table.insert(cframeCache, root.CFrame)
            if #cframeCache > params.cacheSize then
                table.remove(cframeCache, 1)
            end

                                                
            if #cframeCache > 1 and math.random() < params.spoofRate then
                local oldCFrame = cframeCache[1]
                                                                    
                pcall(function()
                    replicatesignal(root, "CFrame", oldCFrame)
                end)
            end
        end)
    end
    ...
    local function stopDesync()
        ...
        if originalFunctions.checkcaller then pcall(hookfunction, checkcaller, originalFunctions.checkcaller) end
        if originalFunctions.getscriptbytecode then pcall(hookfunction, getscriptbytecode, originalFunctions.getscriptbytecode) end
        if originalFunctions.getconnections then pcall(hookfunction, getconnections, originalFunctions.getconnections) end

        table.clear(originalFunctions)
        table.clear(cframeCache)
    end

    Desync = vape.Categories.Blatant:CreateModule({
        Name = "Desync",
        Function = function(callback)
            if callback then startDesync(intensitySettings[CurrentIntensity]) else stopDesync() end
        end,
        Tooltip = "Self explanatory"
    })

    IntensityDropdown = Desync:CreateDropdown({
        Name = "Intensity",
        List = {"Low", "Medium", "High"},
        Default = "Medium",
        Function = function(val)
            CurrentIntensity = val
            if desyncActive then stopDesync() startDesync(intensitySettings[CurrentIntensity]) end
        end
    })

    BypassToggle = Desync:CreateToggle({
        Name = "Enable Bypasses",
        Default = true,
        Function = function(val)
            BypassesEnabled = val
            if desyncActive then stopDesync() startDesync(intensitySettings[CurrentIntensity]) end
